...
const OCCASIONS = [
  {
    ...
    name: "Issiq havo",
    emoji: "☀️",
    seasonPref: ["yoz", "barcha-fasl"],
  },
  {
    id: "sovuq",
    name: "Sovuq havo",
    emoji: "❄️",
    seasonPref: ["qish", "barcha-fasl"],
  },
  {
    id: "ish",
    name: "Ish",
    emoji: "💼",
    categoryPref: ["koylak", "shim", "kostyum", "oyoq-kiyim"],
  },
  {
    id: "universitet",
    name: "Universitet",
    emoji: "🎓",
    categoryPref: ["futbolka", "jinsi", "krossovka"],
  },
  {
    id: "sport",
    name: "Sport",
    emoji: "⚽",
    categoryPref: ["shortik", "futbolka", "krossovka"],
  },
  {
    id: "sayr",
    name: "Sayr",
    emoji: "🚶",
    categoryPref: ["jinsi", "futbolka", "krossovka", "kepka"],
  },
  {
    id: "toy",
    name: "To'y",
    emoji: "🎉",
    categoryPref: ["kostyum", "koylak", "oyoq-kiyim", "soat"],
  },
  {
    id: "mehmon",
    name: "Mehmon",
    emoji: "🍽️",
    categoryPref: ["koylak", "shim", "oyoq-kiyim"],
  },
  {
    id: "sayohat",
    name: "Sayohat",
    emoji: "✈️",
    categoryPref: ["futbolka", "jinsi", "kurtka", "krossovka"],
  },
];
  ...
  return [
    {
      ...
      material: "Paxta",
      condition: "Yaxshi",
      note: "Har kungi kiyim uchun qulay.",
    },
    {
      name: "Ko'k klassik jinsi shim",
      category: "jinsi",
      color: "kok",
      brand: "Levi's",
      price: 320000,
    ...
    },
    {
      ...
      material: "Sherst",
      condition: "Yangi",
      note: "Rasmiy uchrashuvlar uchun.",
    },
    {
      name: "Yashil parka kurtka",
      category: "kurtka",
      color: "yashil",
      brand: "Uniqlo",
      price: 560000,
...
const BADGES = [
  {
    ...
    check: (ctx) => ctx.itemsCount >= 1,
    progress: (ctx) => Math.min(1, ctx.itemsCount / 1),
  },
  {
    id: "wardrobe-master",
    name: "Garderob ustasi",
    icon: "ic-shirt",
    desc: "10 tadan ortiq kiyim qo'shdingiz",
    check: (ctx) => ctx.itemsCount >= 10,
    progress: (ctx) => Math.min(1, ctx.itemsCount / 10),
  },
  {
    id: "favorite-fan",
    name: "Sevimlilar to'plami",
    icon: "ic-heart",
    desc: "5 ta kiyimni sevimliga qo'shdingiz",
    check: (ctx) => ctx.favCount >= 5,
    progress: (ctx) => Math.min(1, ctx.favCount / 5),
  },
  {
    id: "ai-friend",
    name: "AI bilan do'st",
    ...
    check: (ctx) => ctx.aiUsage >= 5,
    progress: (ctx) => Math.min(1, ctx.aiUsage / 5),
  },
  {
    id: "outfit-collector",
    name: "Outfit kolleksioneri",
    icon: "ic-star",
    desc: "3 ta kombinatsiyani saqladingiz",
    check: (ctx) => ctx.outfitsCount >= 3,
    progress: (ctx) => Math.min(1, ctx.outfitsCount / 3),
  },
  {
    id: "calendar-keeper",
    name: "Kalendar ustasi",
    icon: "ic-calendar",
    desc: "7 kunlik kiyinish tarixini yozdingiz",
    check: (ctx) => ctx.wearLogCount >= 7,
    progress: (ctx) => Math.min(1, ctx.wearLogCount / 7),
  },
  {
    id: "streak-week",
    name: "Doimiy foydalanuvchi",
    icon: "ic-award",
    desc: "3 kun ketma-ket ilovaga kirdingiz",
    check: (ctx) => ctx.streak >= 3,
    progress: (ctx) => Math.min(1, ctx.streak / 3),
  },
];

/** Kategoriya/rang/fasl id -> nomi tez topish uchun lookup funksiyalari */
/** Kategoriya/rang/fasl nomini joriy tilda qaytaradi (I18N yuklangan bo'lsa) */
function _localizedName(dictPrefix, id, fallback) {
  if (typeof I18N !== "undefined") {
    const lang = I18N.current();
    const dict = I18N_DICT[lang] || I18N_DICT.uz;
    const key = `${dictPrefix}.${id}`;
    if (dict[key] !== undefined) return dict[key];
  }
  return fallback;
}
function catName(id) {
  const fallback = CATEGORIES.find((c) => c.id === id)?.name || id;
  return _localizedName("category", id, fallback);
}
function colorInfo(id) {
  const base = COLORS.find((c) => c.id === id) || { name: id, hex: "#ccc" };
  return { ...base, name: _localizedName("color", id, base.name) };
}
function seasonName(id) {
  const fallback = SEASONS.find((s) => s.id === id)?.name || id;
  return _localizedName("season", id, fallback);
}
function occasionName(id) {
  const fallback = OCCASIONS.find((o) => o.id === id)?.name || id;
  return _localizedName("occasion", id, fallback);
}
